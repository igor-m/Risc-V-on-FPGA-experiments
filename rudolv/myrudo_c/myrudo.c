...
// myrudo
//--------------------------------------------------------------------------
// igor-m 03/2021: 125kB large program for testing full 128kB of SPRAM
// for UPduino with ice40UP5k
// prints out FLASH vs. SPRAM content
// calculates a number via pretty long unrolled code
...
void delay_(unsigned long cdelay) {
    unsigned long start = read_cycle();
    while (read_cycle() - start < cdelay);
}

void _putchar(char character) { putchar(character); };

//  #### verilog SPI pin defs for UPduino flash
//    assign spics =    leds[4];
...
char spitrx(char Data)
{
  ...
  return ((char)SPIDataIN );                    // Finally return the read data
}
...
static inline void clear_leds(unsigned long leds)
{
    clear_csr(0xbc1, leds);
}

static inline uint32_t read_butts()
{
    return (read_csr(0xfc1));
}
...
/*****/
{
  ...
  // test read from flash at 1MB+ and from spram 0+

    unsigned long datas = 0;
    unsigned long dataf = 0;
    unsigned long words = 0;
    unsigned long wordf = 0;
    unsigned long dummy = 0;
    unsigned long dummyf = 0;
    uint32_t i;

    printf("FLASH TEST STARTS..\n");

    uint32_t address = 0x00100000;      // read the flash from 1MB up
    char *p = 0;                        // read the spram from 0 up

   // Reading the FLASH

    SPI_CK_0; SPI_CS_1; SPI_CS_0;  spitrx(0xAB); SPI_CS_1;     // Wake up from sleep mode
 
    SPI_CK_0; SPI_CS_1; SPI_CS_0;  spitrx(0x03);               // read command 0x03
   
    spitrx(address>>16);        // write flash starting address
    spitrx(address>>8);
    spitrx(address);

    for (uint32_t i=0; i<0x20000; i++) {          // flash read loop - we always read all 128kB
          dummyf = spitrx(0);
          wordf = (wordf >> 8) | ((uint32_t)dummyf << 24);
          dummy = *p++; 
          words = (words >> 8) | ((uint32_t)dummy << 24);
        if ((i&3)==3) {
            dataf = wordf;
            datas = words;
            printf("Address = 0x%0.8x  F = 0x%08x  S = 0x%08x ", i/4, dataf, datas);
            if (datas != dataf) 
                printf("      ############# \n");    // add in case of mismatch
                else
                printf("\n");
         }
    }
